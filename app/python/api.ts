...
/* Generated: 2026-01-30T13:10:24.098483 */
...
export async function addMcpServer(args: { body: AddMCPServerInput }): Promise<MCPServerInfo> {
    const r = await request("add_mcp_server", { body: args.body });
    return { id: r.id, status: r.status, error: r.error, toolCount: r.toolCount, tools: r.tools.map((item: unknown) => ({ id: item.id, name: item.name, description: item.description, inputSchema: item.inputSchema, renderer: item.renderer, rendererConfig: item.renderer_config, requiresConfirmation: item.requires_confirmation })), config: r.config };
}
...
export async function getAvailableTools(): Promise<AvailableToolsResponse> {
    const r = await request("get_available_tools", {});
    return { tools: r.tools.map((item: unknown) => ({ id: item.id, name: item.name, description: item.description, category: item.category, inputSchema: item.inputSchema, renderer: item.renderer, editableArgs: item.editable_args, requiresConfirmation: item.requires_confirmation })) };
}
...
export async function getMcpServers(): Promise<MCPServersResponse> {
    const r = await request("get_mcp_servers", {});
    return { servers: r.servers.map((item: unknown) => ({ id: item.id, status: item.status, error: item.error, toolCount: item.toolCount, tools: item.tools.map((item: unknown) => ({ id: item.id, name: item.name, description: item.description, inputSchema: item.inputSchema, renderer: item.renderer, rendererConfig: item.renderer_config, requiresConfirmation: item.requires_confirmation })), config: item.config })) };
}
...
export async function getToolset(args: { body: ToolsetIdRequest }): Promise<ToolsetDetailInfo> {
    const r = await request("get_toolset", { body: args.body });
    return { id: r.id, name: r.name, version: r.version, description: r.description, enabled: r.enabled, userMcp: r.user_mcp, installedAt: r.installed_at, sourceType: r.source_type, toolCount: r.tool_count, tools: r.tools.map((item: unknown) => ({ toolId: item.tool_id, name: item.name, description: item.description, requiresConfirmation: item.requires_confirmation, enabled: item.enabled })) };
}
...
export async function getWorkspaceManifest(args: { body: WorkspaceManifestRequest }): Promise<WorkspaceManifestResponse> {
    const r = await request("get_workspace_manifest", { body: { chat_id: args.body.chatId, manifest_id: args.body.manifestId } });
    return { id: r.id, chatId: r.chat_id, parentId: r.parent_id, files: r.files, createdAt: r.created_at, source: r.source };
}
...
export async function listToolsets(args: { body?: ListToolsetsRequest }): Promise<ToolsetsResponse> {
    const r = await request("list_toolsets", { body: args.body ? { user_mcp: args.body.userMcp } : undefined });
    return { toolsets: r.toolsets.map((item: unknown) => ({ id: item.id, name: item.name, version: item.version, description: item.description, enabled: item.enabled, userMcp: item.user_mcp, installedAt: item.installed_at, sourceType: item.source_type, toolCount: item.tool_count })) };
}
...
export async function reconnectMcpServer(args: { body: MCPServerId }): Promise<MCPServerInfo> {
    const r = await request("reconnect_mcp_server", { body: args.body });
    return { id: r.id, status: r.status, error: r.error, toolCount: r.toolCount, tools: r.tools.map((item: unknown) => ({ id: item.id, name: item.name, description: item.description, inputSchema: item.inputSchema, renderer: item.renderer, rendererConfig: item.renderer_config, requiresConfirmation: item.requires_confirmation })), config: r.config };
}
...
export async function setToolOverride(args: { body: SetToolOverrideRequest }): Promise<ToolOverrideResponse> {
    const r = await request("set_tool_override", { body: { toolset_id: args.body.toolsetId, tool_id: args.body.toolId, renderer: args.body.renderer, renderer_config: args.body.rendererConfig, name_override: args.body.nameOverride, description_override: args.body.descriptionOverride, requires_confirmation: args.body.requiresConfirmation, enabled: args.body.enabled } });
    return { toolsetId: r.toolset_id, toolId: r.tool_id, renderer: r.renderer, rendererConfig: r.renderer_config, nameOverride: r.name_override, descriptionOverride: r.description_override, requiresConfirmation: r.requires_confirmation, enabled: r.enabled };
}
...
export async function updateMcpServer(args: { body: UpdateMCPServerInput }): Promise<MCPServerInfo> {
    const r = await request("update_mcp_server", { body: args.body });
    return { id: r.id, status: r.status, error: r.error, toolCount: r.toolCount, tools: r.tools.map((item: unknown) => ({ id: item.id, name: item.name, description: item.description, inputSchema: item.inputSchema, renderer: item.renderer, rendererConfig: item.renderer_config, requiresConfirmation: item.requires_confirmation })), config: r.config };
}

export async function updateWorkspaceFile(args: { body: UpdateWorkspaceFileRequest }): Promise<UpdateWorkspaceFileResponse> {
    const r = await request("update_workspace_file", { body: { chat_id: args.body.chatId, path: args.body.path, content: args.body.content } });
    return { manifestId: r.manifest_id, path: r.path };
}
...
export class EventsSocket {
    ...
    connect(): void {
        ...
        this.ws.onmessage = (event) => {
            try {
                ...
                this.listeners.get(message.event)?.forEach(cb => cb(message.data));
            } catch (e) {
    ...
    send<K extends keyof EventsClientEvents>(event: K, data: EventsClientEvents[K]): void {
    ...
    on<K extends keyof EventsServerEvents>(event: K, callback: (data: EventsServerEvents[K]) => void): () => void {
        ...
        return () => this.listeners.get(eventStr)?.delete(cb);
    }
