...
/* Generated: 2026-02-11T14:37:52.604574 */
...
export async function getAgent(args: { body: AgentIdRequest }): Promise<AgentDetailResponse> {
    const _r = await request("get_agent", { body: args.body });
...
export async function listAgents(): Promise<AgentsListResponse> {
    const _r = await request("list_agents", {});
    return { agents: _r.agents.map((item: unknown) => ({ id: item.id, name: item.name, description: item.description, icon: item.icon, previewImage: item.preview_image, includeUserTools: item.include_user_tools, createdAt: item.created_at, updatedAt: item.updated_at })) };
}
 ...
 * @returns URL string for the static file
 */
