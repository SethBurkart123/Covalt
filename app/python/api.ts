...
/* Generated: 2026-02-22T18:18:49.528012 */
...
export async function addMcpServer(args: { body: AddMCPServerInput }): Promise<MCPServerInfo> {
    ...
    return { id: _r.id, status: _r.status, error: _r.error, toolCount: _r.toolCount, tools: _r.tools.map((item: any) => ({ id: item.id, name: item.name, description: item.description, inputSchema: item.inputSchema, renderer: item.renderer, rendererConfig: item.renderer_config, requiresConfirmation: item.requires_confirmation })), config: _r.config };
}
...
export async function getAvailableTools(): Promise<AvailableToolsResponse> {
    ...
    return { tools: _r.tools.map((item: any) => ({ id: item.id, name: item.name, description: item.description, category: item.category, inputSchema: item.inputSchema, renderer: item.renderer, editableArgs: item.editable_args, requiresConfirmation: item.requires_confirmation })) };
}
...
export async function getMcpServers(): Promise<MCPServersResponse> {
    ...
    return { servers: _r.servers.map((item: any) => ({ id: item.id, status: item.status, error: item.error, toolCount: item.toolCount, tools: item.tools.map((item: any) => ({ id: item.id, name: item.name, description: item.description, inputSchema: item.inputSchema, renderer: item.renderer, rendererConfig: item.renderer_config, requiresConfirmation: item.requires_confirmation })), config: item.config })) };
}
...
export class EventsSocket {
    ...
    onMcpStatus(callback: (data: McpServerStatus) => void): () => void {
        return this.on("mcp_status", (_d) => { const _raw = _d as any; return callback({ id: _raw.id, status: _raw.status, error: _raw.error, toolCount: _raw.tool_count, oauthStatus: _raw.oauth_status, oauthProviderName: _raw.oauth_provider_name, authHint: _raw.auth_hint, config: _raw.config }); });
    }

    onMcpServers(callback: (data: McpServersSnapshot) => void): () => void {
        return this.on("mcp_servers", (_d) => { const _raw = _d as any; return callback({ servers: _raw.servers.map((item: any) => ({ id: item.id, status: item.status, error: item.error, toolCount: item.tool_count, oauthStatus: item.oauth_status, oauthProviderName: item.oauth_provider_name, authHint: item.auth_hint, config: item.config })) }); });
    }
