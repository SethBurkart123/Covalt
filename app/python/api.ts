...
/* Generated: 2026-02-19T13:58:22.601685 */
...
export class EventsSocket {
    ...
    onMcpServers(callback: (data: McpServersSnapshot) => void): () => void {
        return this.on("mcp_servers", (_d) => callback({ servers: _d.servers.map((item: any) => ({ id: item.id, status: item.status, error: item.error, toolCount: item.tool_count, oauthStatus: item.oauth_status, oauthProviderName: item.oauth_provider_name, authHint: item.auth_hint, config: item.config })) }));
    }
