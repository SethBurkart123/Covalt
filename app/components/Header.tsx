...
function HeaderInner() {
  const { title, leftContent, rightContent, floating, rightOffset } =
    usePageTitle();
  const { open: sidebarOpen } = useSidebar();
  const isMacElectrobun = useIsElectrobunMac();
  const leftPadding =
    isMacElectrobun && !sidebarOpen ? TITLEBAR_OFFSET_PX : DEFAULT_PADDING_PX;
  ...
  if (floating) {
    return (
      <motion.header
        ...
        initial={false}
        animate={{
          paddingRight: rightOffset + DEFAULT_PADDING_PX,
