...
export default function ModelSelector({ selectedModel, setSelectedModel, models }: ModelSelectorProps) {
  ...
  const groupedModels = useMemo(() => {
    ...
    const providerGroupList = Array.from(providerGroups.entries()).map(([provider, modelList]) => ({
      provider,
      models: modelList,
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="flex flex-shrink-0 items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium h-9 justify-between min-w-20"
        >
          {selectedModelInfo ? (
        ...
        <Command className="rounded-2xl">
          ...
          <CommandList>
            ...
            {groupedModels.map((group) => {
              const providerName = group.isRecent ? "Recent" : (group.providerDef?.name || group.provider)
