...
function EmojiPickerSearch({
  ...
  return (
    <div
      className={cn("flex h-11 items-center gap-2 border-b px-3", className)}
      data-slot="emoji-picker-search-wrapper"
    >
      <SearchIcon className="size-4 shrink-0 opacity-50" />
      <EmojiPickerPrimitive.Search
        className="outline-hidden placeholder:text-muted-foreground flex h-11 w-full rounded-md bg-transparent py-3 text-sm disabled:cursor-not-allowed disabled:opacity-50"
...
function EmojiPickerContent({
  ...
  return (
    <EmojiPickerPrimitive.Viewport
      className={cn("outline-hidden relative flex-1 min-h-0", className)}
      ...
      <EmojiPickerPrimitive.ActiveEmoji>
        {({ emoji }) =>
          emoji ? (
            ...
            <span className="text-muted-foreground ml-1.5 flex h-8 items-center truncate text-sm">
              Select an emoji…
            </span>
          )
        }
