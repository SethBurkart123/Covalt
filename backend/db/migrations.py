...
def run_migrations() -> None:
    ...
    try:
        with engine.connect() as conn:
            ...
            if table_def and "agent_config" not in table_def[0]:
                ...
                print(
                    "[db] Running migration: Adding agent_config column to chats table"
                )
                conn.execute(
        ...
        with engine.connect() as conn:
            ...
            if table_def:
                ...
                if "parent_message_id" not in table_def[0]:
                    print(
                        "[db] Running migration: Adding parent_message_id column to messages table"
                    )
                    conn.execute(
                        sqlalchemy.text(
                ...
                if "is_complete" not in table_def[0]:
                    print(
                        "[db] Running migration: Adding is_complete column to messages table"
                    )
                    conn.execute(
                        sqlalchemy.text(
                            "ALTER TABLE messages ADD COLUMN is_complete INTEGER DEFAULT 1 NOT NULL"
                        )
                    )
                    needs_migration = True
                ...
                if "sequence" not in table_def[0]:
                    print(
                        "[db] Running migration: Adding sequence column to messages table"
                    )
                    conn.execute(
                        sqlalchemy.text(
                            "ALTER TABLE messages ADD COLUMN sequence INTEGER DEFAULT 1 NOT NULL"
                        )
                    )
                    needs_migration = True
                ...
                if "model_used" not in table_def[0]:
                    print(
                        "[db] Running migration: Adding model_used column to messages table"
                    )
                    conn.execute(
                        sqlalchemy.text(
    ...
    try:
        with engine.connect() as conn:
            ...
            if table_def and "active_leaf_message_id" not in table_def[0]:
                print(
                    "[db] Running migration: Adding active_leaf_message_id column to chats table"
                )
                conn.execute(
                    sqlalchemy.text(
                        "ALTER TABLE chats ADD COLUMN active_leaf_message_id TEXT"
                    )
                )
                conn.commit()
    ...
    try:
        with engine.connect() as conn:
            ...
            if (
                table_def
                and " extra " not in (table_def[0] or "")
                and '"extra"' not in (table_def[0] or "")
            ):
                print(
                    "[db] Running migration: Adding extra column to provider_settings table"
                )
                conn.execute(
                    sqlalchemy.text(
                        "ALTER TABLE provider_settings ADD COLUMN extra TEXT"
                    )
                )
                conn.commit()
        ...
        with engine.connect() as conn:
            ...
            if not models_table_exists:
                if old_table_exists:
                    ...
                    print(
                        "[db] Running migration: Migrating model_settings to models table"
                    )
